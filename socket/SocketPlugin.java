...
public class SocketPlugin extends Plugin implements Runnable {
    ...
    public void closeConnection(boolean verbose) {
        ...
        if (verbose)
            this.clientThread.invoke(() -> client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "<col=008000>Any active socket server connections were closed.", null));
    }
    ...
    public void run() {
        ...
        System.out.println("[SocketPlugin] [Send] " + System.currentTimeMillis());
        System.out.println(joinCommand.toString(4));
        this.socketOutput.println(joinCommand.toString());

        this.clientThread.invoke(() -> client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "<col=008000>Connection to the socket server was successfully established.", null));
        this.isConnecting = false;
        ...
        try {
            ...
            while (!this.isStopping // Plugin is not stopped.
                    ...
                    && (ln = this.socketInput.readLine()) != null) { // Plugin has received new input.
                try {
                    String secret = SocketAESEncryption.decrypt(this.config.getSalt(), ln);
                    if (secret == null) // We ignore bad packets.
                        continue;

                    JSONObject payload = new JSONObject(secret); // We assume everything is JSON. If it's not, the packet wasn't meant for us.
                    System.out.println("[SocketPlugin] [Receive] " + System.currentTimeMillis());
                    System.out.println(payload.toString(4));
                    this.clientThread.invoke(() -> eventBus.post(new SocketReceivePacket(payload))); // Dispatch the payload via an event handler.
                } catch (JSONException ignore) {
                }
            }
        } catch (Exception e) {
            if (!this.isStopping) { // We don't print the error if the plugin is stopping -> It's not a bug!
                e.printStackTrace();
    ...
    public void onGameTick(GameTick event) {
        if (client.getGameState() == GameState.LOGGED_IN) {
            if (this.socket == null && !this.isConnecting && System.currentTimeMillis() >= this.nextConnection) { // Attempt a new connection.
                this.isConnecting = true;

                this.client.addChatMessage(ChatMessageType.GAMEMESSAGE, "", "<col=008000>Attempting to connect to the socket server...", null);
